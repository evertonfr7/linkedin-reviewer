...
const ALLOWED_MHTML_TYPES = ['message/rfc822', 'multipart/related', 'text/html'];
const ACCEPT_STRING = '.mhtml,.mht,.html,.htm,message/rfc822,multipart/related,text/html';

export default function MhtmlUpload({ onSubmit, onCancel, isLoading }: MhtmlUploadProps) {
  const [mhtmlFile, setMhtmlFile] = useState<{ name: string; size: number; file: File } | null>(null);
  ...
  const isAllowedFile = useCallback((file: File) => {
    ...
    return name.endsWith('.mhtml') || name.endsWith('.mht') ||
           name.endsWith('.html') || name.endsWith('.htm') ||
           ALLOWED_MHTML_TYPES.includes(file.type);
  }, []);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    const validFile = droppedFiles.find(file => isAllowedFile(file));

    if (!validFile) {
      setUploadError('Por favor, envie um arquivo MHTML ou MHT do LinkedIn.');
      return;
    }
    ...
    setMhtmlFile({ name: validFile.name, size: validFile.size, file: validFile });
    setPreview(null);
  }, [isAllowedFile]);
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isAllowedFile(file)) {
      setUploadError('Por favor, selecione um arquivo MHTML ou MHT.');
      return;
    }
    ...
    setMhtmlFile({ name: file.name, size: file.size, file });
    setPreview(null);
    e.target.value = '';
  }, [isAllowedFile]);
      ...
      try {
        ...
        if (data.success) {
          setProfilePhoto({
            ...
            size: file.size,
            base64,
            hasPhoto: data.photoResult?.hasPhoto,
            description: data.photoResult?.description,
          });
        } else {
          setPhotoError(data.error || 'Erro ao analisar foto.');
        }
      } catch {
        setPhotoError('Erro ao conectar com o servidor para analisar a foto. Tente novamente.');
      } finally {
  ...
  const handleProcess = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append('file', mhtmlFile.file);

      const response = await fetch('/api/mhtml', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.success && data.preview) {
        ...
        setUploadError(data.error || 'Erro ao processar MHTML');
      }
    } catch {
  ...
  const handleConfirm = () => {
    if (preview) {
      onSubmit({
        ...
        profilePhoto: profilePhoto?.hasPhoto ? 'analysed' : null,
        photoDescription: profilePhoto?.description || null,
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-headline font-bold text-xl text-on-surface">
              Upload do Perfil LinkedIn
            </h2>
            <p className="text-sm text-on-surface-variant">
              Exporte seu perfil como MHTML usando o Blink
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              {isAnalyzingPhoto ? (
                ...
                <p className="text-xs text-green-600 mt-1">Foto analisada com sucesso pela IA</p>
              ) : (
                <p className="text-xs text-on-surface-variant mt-1">JPG, PNG ou WebP. Maximo 5MB.</p>
              )}
            </div>
          </div>
          {photoError && (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                <p className="font-medium text-on-surface">
                  {mhtmlFile ? mhtmlFile.name : 'Arraste o arquivo MHTML do LinkedIn aqui'}
                </p>
                <p className="text-sm text-on-surface-variant mt-1">
                  {mhtmlFile ? formatFileSize(mhtmlFile.size) : 'ou clique para selecionar'}
                </p>
              </div>
            </div>
          </label>
          ...
          <div className="flex items-center gap-3">
            {!preview && (
              <button
                ...
                disabled={!mhtmlFile || isProcessing}
        ...
        <div className="mt-6 bg-surface-container-lowest rounded-2xl p-6 shadow-lg">
          ...
          {preview && (
            <div className="mt-6 p-4 bg-surface-container-low rounded-xl border border-outline-variant">
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-tertiary flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
                <div className="flex-1">
                  <label htmlFor="manualInfo" className="block text-sm font-medium text-on-surface mb-2">
                    Informações adicionais (opcional)
                  </label>
                  <p className="text-xs text-on-surface-variant mb-2">
                    Caso algum campo não tenha sido extraído corretamente, adicione manualmente abaixo.
                  </p>
                  <textarea
                    id="manualInfo"
                    value={manualInfo}
                    ...
                    placeholder="Ex:&#10;Projetos:&#10;- Projeto A - descrição&#10;- Projeto B - descrição&#10;&#10;Idiomas:&#10;- Português - Nativo&#10;- Inglês - Avançado"
                    className="w-full h-32 p-3 bg-surface text-on-surface border border-outline-variant rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-secondary/50 resize-none"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="flex items-start gap-3 text-sm text-on-surface-variant">
          ...
          <div>
            ...
            <ol className="mt-2 space-y-1 list-decimal list-inside">
              ...
              <li>Use o navegador Blink (ou Netscape)</li>
              <li>Salve a página como MHTML (Arquivo &gt; Salvar como)</li>
              <li>Selecione o arquivo aqui</li>
              <li>Clique em "Processar MHTML" para extrair os dados</li>
            </ol>
          </div>
        </div>
      </div>
